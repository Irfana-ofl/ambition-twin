...
import { askGateway, roadmapGateway } from "./twin-ai.server";
...
const RoadmapSchema = z.object({ context: z.string().min(1).max(12000) });

export const generateRoadmap = createServerFn({ method: "POST" })
  .inputValidator((input: unknown) => RoadmapSchema.parse(input))
  .handler(async ({ data }) => {
    try {
      const phases = await roadmapGateway(data.context);
      if (!phases) return { ok: false as const, reason: "unavailable" as const };
